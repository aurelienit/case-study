...
def check_subDir(Curpath):
    ...
    print "\n***********************************************************************************"
    print "***********************************************************************************\n"
    question = raw_input("Do you want to see the files in the sub-directory(ies)? Enter Yes or No\n")
...
def Error_handling(mountpoint):
    try:
        ...
        if err.errno == errno.EACCES:
            ...
            sys.exit(1)
        # Display Error Message if Mountpoint does not exist
        if err.errno == errno.ENOENT:
            print "\nOops! This mountpoint " + mountpoint + " does not exist! Try again!\n"
            sys.exit(1)
        # Display Error Message if not a valide Directory
